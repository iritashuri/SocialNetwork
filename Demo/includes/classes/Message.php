...
class Message
{
    ...
    public function getLatestMessages($userLoggedIn, $user2)
    {
        ...
        OR (user_from='$userLoggedIn' AND user_to='$user2') ORDER BY id DESC LIMIT 1");
        $row = mysqli_fetch_array($query);
    ...
    public function getConvos()
    {
        ...
        while ($row = mysqli_fetch_array($query)) {
            ...
            if (!in_array($user_to_push, $convos) && !empty($user_to_push)) {
                array_push($convos, $user_to_push);
            }
        }
        foreach ($convos as $username) {
            $user_found_obj = new User($this->con, $username);
            $latest_messages_details = $this->getLatestMessages($userLoggedIn, $username);
            ...
            $return_string .= "<a href=messages.php?u=$username> <div class='user_found_messages'>
                                ...
                                style='border-radius: 5px; margin-right: 5x;'>" . $user_found_obj->getFirstAndLastName() .
                "<span class='timestamp_smaller' id='grey'><br>" . $latest_messages_details[2] . "</span>
