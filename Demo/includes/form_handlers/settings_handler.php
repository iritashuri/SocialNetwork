...
if (isset($_POST['update_details'])) {
    ...
    $user_to_query = new User($con, $userLoggedIn);
    $user_to_query_mail = $user_to_query->getMail();

    $email_check = mysqli_query($con, "SELECT * FROM users WHERE email='$user_to_query_mail'");
    ...
    if (md5($old_password) == $db_password) {
        if ($new_password_1 == $new_password_2) {
            if (strlen($new_password_1) <= 4) {
                $password_message = "Sorry, your password must be greater then 4 characters<br><br>";
            } else {
