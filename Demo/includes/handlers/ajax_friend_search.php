...
//If query contain an underscore, assume user is reacing for username
if (strpos($query, "_") !== false) {
    $usersReturned = mysqli_query($con, "SELECT * FROM users WHERE username LIKE '$query%' AND user_closed='no' LIMIT 8");
} //If there are 2 words  assume user search for first and kast name
...
} //If  query has 1 word only serch for first name and last name
else {
    //if the input is "*" search for all
    if ($names[0] == "*" || $query == "*") {
        $usersReturned = mysqli_query($con, "SELECT * FROM users WHERE user_closed='no' LIMIT 8");
    } else {
